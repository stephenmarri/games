class Solver {
    ...
    async backTracking() {
        ...
        console.log(`Back Tracking : empty items left - ${bTquestions}`);
        ...
        while (i < bTquestions) {
            ...
            if (!isFilled) {
                if (i == 0) {
                    this.isTheBoardSolved("backtracking")
                    return;
                } else {
        ...
        // final steps
        this.solvedCount += i;
        console.log('Back Tracking : iterations done - ', iterations);
        console.log(`Back Tracking : ${i} filled out of ${bTquestions}`);        
        // final validation and exit with result
        this.isTheBoardSolved("backtracking")
    }
    ...
    isTheBoardSolved(stage) {
        if (this.solvedCount == this.questionsCount) {
            this.isBoardSolved = true;
            console.log(stage,": board solved");
            ...
            if (isSolutionValid){
                ...
                console.log('Solver process complete. But board is invalid somehow.Sorry 😥');
                alert('Process complete, but solution is invalid. Sorry 😥')
            }

        } else if (stage.toString().replace(' ', '').toLowerCase() == "backtracking") {
            this.isBoardSolved = false;
